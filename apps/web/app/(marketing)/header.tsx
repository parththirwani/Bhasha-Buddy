...
export const Header = () => {
  ...
  return (
    <header className="h-20 w-full border-b-2 border-slate-800 px-4 dark:border-zinc-600">
      <div className="lg:max-w-screen-lg mx-auto flex items-center justify-between h-full">
        <div className="flex items-center gap-x-3">
          <Link href="/" passHref>
        ...
        {session?.user?(<div className="flex items-center gap-x-4">
            <Button size="lg" className='dark:bg-violet-800 dark:text-white' onClick={() => signOut()}>Logout</Button>  
        </div>):(<div className="flex items-center gap-x-4">
            <Button size="lg" className='dark:bg-violet-800 dark:text-white' onClick={() => signIn()}>Login</Button>  
        </div>)}
      </div>
    </header>
  );
